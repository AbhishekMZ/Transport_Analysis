...
Usage:
    ...
    # Load a local Pickle network using an absolute path
    python scripts/load_data.py load-pickle --file "C:\path\to\your\graph_2023.pkl" --year 2023

    # Load all local data (GTFS and yearly Pickle files)
    python scripts/load_data.py load-all-pickle --start-year 2023 --end-year 2024 --gtfs-dir "bmtc" --path-template "C:\path\to\graph_{year}.pkl"
"""
...
def load_pickle_network(file_path_str: str, year: int):
    """
    Loads a pickle file. If it contains a NetworkX graph, it's converted to a
    GeoDataFrame before loading. Handles absolute and relative paths.
    """
    logger.info(f"--- Starting Local Pickle Network Load for {year} from '{file_path_str}' ---")
    
    file_path = Path(file_path_str)
    if not file_path.is_absolute():
        file_path = settings.GEOJSON_DIR / file_path_str
    ...
    try:
        ...
        data_obj = pd.read_pickle(file_path)
        
        gdf = None
        if isinstance(data_obj, gpd.GeoDataFrame):
            logger.info("Pickle file contains a GeoDataFrame. Proceeding directly.")
            gdf = data_obj
        elif isinstance(data_obj, nx.Graph):
            logger.info("Pickle file contains a NetworkX graph. Converting to GeoDataFrame...")
            
            # *** FIX: Check if the graph has edges before trying to convert ***
            if data_obj.number_of_edges() == 0:
                logger.warning(f"Graph in {file_path.name} contains no edges. Skipping this file.")
                return True # Return True to not halt a batch process

            if 'crs' not in data_obj.graph:
                logger.warning("Graph is missing CRS information. Assuming WGS84 (epsg:4326).")
                data_obj.graph['crs'] = 'epsg:4326'
            
            gdf = ox.graph_to_gdfs(data_obj, nodes=False, edges=True)
            logger.info("Conversion to GeoDataFrame successful.")
        else:
            logger.error(f"Pickle file at {file_path} contains an unsupported object type: {type(data_obj)}")
            return False

        logger.info(f"Processing {len(gdf)} features from the data.")
...
def load_all_local_data(start_year: int, end_year: int, gtfs_dir: str, path_template: str):
    """Loads GTFS and all yearly Pickle network files for a given range of years."""
    ...
    for year in range(start_year, end_year + 1):
        ...
        pickle_file_path = path_template.format(year=year)
        load_pickle_network(pickle_file_path, year)
...
def main():
    ...
    pickle_parser.add_argument('--file', required=True, help='Filename or absolute path of the .pkl file.')
    pickle_parser.add_argument('--year', type=int, required=True, help='The year this data represents.')

    all_pickle_parser = subparsers.add_parser('load-all-pickle', help='Load all local GTFS and Pickle data for a range of years.')
    all_pickle_parser.add_argument('--start-year', type=int, required=True, help='The first year in the range.')
    all_pickle_parser.add_argument('--end-year', type=int, required=True, help='The last year in the range.')
    all_pickle_parser.add_argument('--gtfs-dir', required=True, help='Name of the GTFS directory to use for all years.')
    all_pickle_parser.add_argument('--path-template', required=True, help='Absolute path template for pickle files, with {year} as a placeholder.')
    ...
    elif args.command == 'load-all-pickle':
        load_all_local_data(args.start_year, args.end_year, args.gtfs_dir, args.path_template)
